/**
 * @author based on SmartCam for Android, ARGB8888 support by sven@killig.de
 */
...
#include "com_smartcam_webcam_video_JpegHandler.h"
#include "LogUtil.h"
...
JNIEXPORT jbyteArray JNICALL Java_com_smartcam_webcam_video_JpegHandler_initRgb(JNIEnv * env, jobject obj,
jint frameBufferSize, jint frameWidth, jint frameHeight)
{
	LOGV("!!!!!!!!!!!!!!!!!!!!!!!!!! initRgb !!!!!!!!!\n");
	enc = new CJpegEncoder();
	enc->InitRGB(frameBufferSize, frameWidth, frameHeight);
...
JNIEXPORT jbyteArray JNICALL Java_com_smartcam_webcam_video_JpegHandler_initYuv(
...
/*JNIEXPORT jbyteArray JNICALL Java_com_smartcam_webcam_video_JpegHandler_encodeYUV420SP(JNIEnv * env, jobject obj, jbyteArray data)
{
	jbyte* inArray = NULL;
	inArray = env->GetByteArrayElements(data, JNI_FALSE);
	enc->EncodeYUV420SP((const unsigned char*) inArray, 320, 240);
	env->ReleaseByteArrayElements(data, inArray, 0);
...
JNIEXPORT jbyteArray JNICALL Java_com_smartcam_webcam_video_JpegHandler_encodeRGB24(JNIEnv * env, jobject obj, jbyteArray data)
{
	jint len = env->GetArrayLength(data);
	jbyte* copyBuf = (jbyte*)enc->GetFrameBuffer();
	env->GetByteArrayRegion(data, 0, len, copyBuf);

	enc->EncodeRGB24((const unsigned char*) copyBuf);
...
void CJpegEncoder::InitRGB(int frameBufferLen, int frameW, int frameH)
{
	if(cinfoCreated)
		return;
	cinfoCreated = TRUE;
	...
	cinfo.input_components = 3;//1: only left 1/3rd updated//4 -> Bogus input colorspace
	cinfo.in_color_space = JCS_RGB;//JCS_GRAYSCALE: only left 1/3rd updated//JCS_RGBA_8888 -> Bogus input colorspace
...
int CJpegEncoder::EncodeRGB24(const unsigned char* srcBuff, int width, int height)
{
	...
	int row_stride = width * /*3*/4;			// physical row width in buffer
	const unsigned char* srcBuffPtr;
	int crtRow = 0;
...
srcBuffPtr=srcBuff + crtRow * row_stride;
		for(int w = 0; w < width; w++)
		{
			lineRGB24[w*3]     = *(srcBuffPtr++);
			lineRGB24[w*3 + 1] = *(srcBuffPtr++);
			lineRGB24[w*3 + 2] = *(srcBuffPtr++);
			srcBuffPtr++;
		}
